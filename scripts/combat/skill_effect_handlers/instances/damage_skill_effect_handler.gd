...
func handle(config: SkillEffectConfig, targets: Array[Character], cast_context: SkillCastContext, execution_context: SkillExecutionContext) -> void:
	...
	var damage_scaled_by_stat: int = scale_effect_by_stat(caster, dmg_config.damage, config)
	
	for target: Character in targets:
		execution_context.total_damage_done += target.get_hp().receive_damage(caster, damage_scaled_by_stat)
